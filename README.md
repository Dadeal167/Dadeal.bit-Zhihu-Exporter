...
| 🖨️ PDF 导出     | 基于 Playwright 无头浏览器渲染，排版精美、支持数学公式与内嵌配图          |
| 🧮 公式离线渲染     | MathJax 已内置打包，断网也能正常渲染 PDF 公式                        |
| 🖼️ 图片本地化      | 并发下载文章内所有图片至本地 `assets/` 目录，已存在则增量跳过               |
| ⏭️ 断点续传        | 按格式(md/pdf)分别记录下载状态，重启后自动只补缺失的格式                    |
| 🛑 任务停止        | 运行中可随时停止，已完成进度自动保存，下次继续                             |
| 🖥️ 浏览器复用      | 批量任务共享一个渲染引擎，PDF 阶段提速数倍                              |
| ⚙️ 设置面板        | 休眠开关/范围、图片并发、PDF渲染等待、自定义输出目录、托盘行为等均可配置        |
| 🎨 蓝白主题        | 现代 Windows 风低饱和浅蓝 QSS，无边框圆角半透明窗口 + 柔和光晕                    |
| ✨ BASpark 动效    | 光标拖尾/菱形粒子/点击波纹(移植自 index.html)，动态光晕背景，按钮 hover/press 微动画 |
| 🔗 剪贴板监听      | 复制知乎链接自动填入输入框并托盘提醒                                  |
| 🖱️ 拖拽导入        | 把链接或 TXT 文件直接拖进窗口即可导入                                 |
| ⏱️ 进度 ETA       | 实时显示已用时间与预计剩余时间                                       |
| 🤖 本地 AI        | 基于 Ollama 生成文章摘要与学科分类(写入 YAML + 分类索引)               |
| 📚 Obsidian 直通  | 导出的 Markdown 与图片自动同步进 Obsidian 库                        |
...
> 自动完成: 语法检查 → PyInstaller 打包 → Inno Setup 生成安装包到 `Output/`。
